...
{   
    ...
    */
    public function luckythirtysix()
    {
		...
		$data['result']=verdictBSDBZ($input_array); //判断
		$data['create_time']=time();
		$data['status']=2;//已开奖
		$data['period']='';
    ...
    *采用北京福彩中心快乐8数据，与PC蛋蛋结果相同，每5分钟一期，每天179期，每天0-9点暂停开奖
    */
    public function dandanwaiwei28()
    {
        $str='01,23,24,26,30,31,36,38,41,44,45,47,62,67,70,71,73,75,77,78';
        $arr=explode(',',$str);
        ...
        $data['open_time']=$now;
        $data['desc']=$str;
        $data['endnum']=$a.','.$b.','.$c;//位数，如987，234
        $data['result']=$a+$b+$c; //相加结果判断
        $data['create_time']=time();
        $data['status']=2;//已开奖
        $data['period']='';
    
        $row=Db::name('game_ddww28')->where('status',1)->order('id asc')->find();
 
        $data_=array();
        if(empty($row['id'])){//初始
            Db::name('game_ddww28')->insert($data);
            // $lastid=Db::name('game_xybjl')->getLastInsID();
            $data_ = [
                [ 'open_time' => strtotime("+1minute"),'status'=>1,'period'=>'thisTimes'],
                [ 'open_time' => strtotime("+2minute"),'status'=>1,'period'=>''],
            ...
            Db::name('game_ddww28')->insertAll($data_);
        }elseif($row['open_time']<time()){//网络中断
            Db::startTrans();
            try{
                $id=$row['id'];
                Db::name('game_ddww28')->where('id',$id)->update($data);
                
                $data_ =[ 'open_time' => strtotime("+1minute"),'period'=>'thisTimes'];
                Db::name('game_ddww28')->where('id',$id+1)->update($data_);
                unset($data_);
                $data_ = [ 'open_time' => strtotime("+2minute")];
                Db::name('game_ddww28')->where('id',$id+2)->update($data_);
                unset($data_);
                $data_ = [ 'open_time' => strtotime("+3minute")];
                Db::name('game_ddww28')->where('id',$id+3)->update($data_);
                unset($data_);

                $data_ = [ 'open_time' => strtotime("+4minute"),'status'=>1];
                Db::name('game_ddww28')->insert($data_);
                // 提交事务
                Db::commit(); 
            ...
            try{
                ...
                //更新下一期为当期开奖期
                $next_id=$id+1;
                Db::name('game_ddww28')->where('id',$next_id)->update(['period'=>'thisTimes']);
                //同时插入一条记录
                $data_ = [ 'open_time' => strtotime("+4minute"),'status'=>1];
                Db::name('game_ddww28')->insert($data_);
                //自动投注用户投注$next_id这一期
                ...
                foreach($userautomodes as $v){
                    ...
                    //更改对应游戏表的投注总额，投注人数
                    Db::name('game_ddww28')->where('id',$next_id)->setInc('bet_num',1);
                    Db::name('game_ddww28')->where('id',$next_id)->setInc('total_money',$bidmoney);
                }
                // 提交事务
                Db::commit(); 
    ...
    *采用北京福彩中心快乐8数据，与PC蛋蛋结果相同，每5分钟一期，每天179期，每天0-9点暂停开奖
    */
    public function dandan36()
    {
        $str='02,04,07,08,18,22,25,30,35,36,43,49,50,53,59,66,69,71,74,75';
        $arr=explode(',',$str);
		...
		$data['desc']=$str;
        $data['endnum']=$a.','.$b.','.$c;//位数，如987，234
		$input_array=explode(',',$data['endnum']);
		$data['result']=verdictBSDBZ($input_array); //判断
		$data['create_time']=time();
		$data['status']=2;//已开奖
		$data['period']='';
    
        $row=Db::name('game_dd36')->where('status',1)->order('id asc')->find();
 
        $data_=array();
        if(empty($row['id'])){//初始
        	Db::name('game_dd36')->insert($data);
        	// $lastid=Db::name('game_xybjl')->getLastInsID();
        	$data_ = [
			    [ 'open_time' => strtotime("+1minute"),'status'=>1,'period'=>'thisTimes'],
			    [ 'open_time' => strtotime("+2minute"),'status'=>1,'period'=>''],
			...
			Db::name('game_dd36')->insertAll($data_);
        }elseif($row['open_time']<time()){//网络中断
        	Db::startTrans();
            try{
	            $id=$row['id'];
		        Db::name('game_dd36')->where('id',$id)->update($data);
	            
	        	$data_ =[ 'open_time' => strtotime("+1minute"),'period'=>'thisTimes'];
	        	Db::name('game_dd36')->where('id',$id+1)->update($data_);
	        	unset($data_);
				$data_ = [ 'open_time' => strtotime("+2minute")];
				Db::name('game_dd36')->where('id',$id+2)->update($data_);
	        	unset($data_);
				$data_ = [ 'open_time' => strtotime("+3minute")];
				Db::name('game_dd36')->where('id',$id+3)->update($data_);
	        	unset($data_);

				$data_ = [ 'open_time' => strtotime("+4minute"),'status'=>1];
		        Db::name('game_dd36')->insert($data_);
		        // 提交事务
                Db::commit(); 
            ...
            try{
	            ...
	            //更新下一期为当期开奖期
	            $next_id=$id+1;
	            Db::name('game_dd36')->where('id',$next_id)->update(['period'=>'thisTimes']);
	            //同时插入一条记录
	            $data_ = [ 'open_time' => strtotime("+4minute"),'status'=>1];
	            Db::name('game_dd36')->insert($data_);
	            //自动投注用户投注$next_id这一期
                ...
                foreach($userautomodes as $v){
                    ...
                    //更改对应游戏表的投注总额，投注人数
                    Db::name('game_dd36')->where('id',$next_id)->setInc('bet_num',1);
                    Db::name('game_dd36')->where('id',$next_id)->setInc('total_money',$bidmoney);
                }
                // 提交事务
                Db::commit(); 
